...
for ubar=0.8:0.1:1.5
    ubar
for ustep = 0.350:-0.01:0.200 
    ustep
 for cs = 50:-1:5
  for cw=5:1:50    
  %while cw < 100
   ...
   % ubar
%   pause
bound_energy = fzeros4('QW2GaN',0.001,(ubar-0.001),0.001); %Call fzeros3
%pause
% product = product_e(bound_energy);
    if (length(bound_energy)>=4)
        ...
        if E21<=0.242 && E21 >= 0.238 && E32<=0.242 && E32 >= 0.238 && E43<=0.242 && E43 >= 0.238  %Resonance condition
        cw_data = [cw_data cw]         
        cs_data = [cs_data cs]
...
display(e43_data);
 ...
 for i = 1:length(Vs_data)
     ...
     product = thirdharmonic_product_eGaN(bound_energy);
